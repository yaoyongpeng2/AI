...
def average(values):
    ...
    这是普通文本行，不会被解析为测试。           <- 普通文本（非测试）
    
    >>> x = 5                    # 测试代码行(以 `>>>` 开头)
    >>> y = 10                   # 测试代码（与上一句独立,仍以 `>>>` 开头,属于代码块）
    >>> print(x + y)             # 下一行15是预期结果，严格匹配，只能这里加注释
    15 

    以下这一行以 `...` 开头，但前面没有 `>>>` 或代码块，因此被视为普通文本：
    ... 这是普通文本中的省略号，不是测试代码！  <- 普通文本（非测试）
    
    >>> for i in range(3):       # 测试代码
    ...     print(i)             # 测试代码续行(与上一句相关,以 `...` 开头，属于代码块)
     0
    1
    2
    """
...
import doctest
#测试代码标识是'>>> '（有个空格），或’... '详细见文档字符串，必须是#注释
#doctest.NORMALIZE_WHITESPACE 可忽略输出结果前后空白字符的差别
doctest.testmod(optionflags=doctest.NORMALIZE_WHITESPACE)#全通过
#对比：输出有空白字符的失败：
#例1：print(x + y)之下一行是15 （后面有个空格）
#for循环...     print(i)下一行是 0（0之前有空格）
doctest.testmod()#失败2个，for循环那个算3个，第一个失败
